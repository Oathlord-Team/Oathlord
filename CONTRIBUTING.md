# Oathlord contribution guidelines and standards
...
1. All new C# code must go in the `Content.Oathlord.*` modules.
2. Try to only add events to `Content.Oathlord.Common`, to allow for deocoupling for upstream's and your logic.
...
All resources go in a `_Oathlord` subdirectory inside the resource's folder, e.g. `Resources/Prototypes/_Oathlord` for all YML prototypes.
...
Doing this eliminates the need for upstream code to be dependent on your random systems, or those random systems to have any code in `Oathlord.Common`.
...
All new entity prototypes must have `categories: [ ForkFiltered ]` set, otherwise they will not show up in the entity spawn menu.
```yml
- type: entity
  id: MyNewItem
  name: my new item
...
For single line changes use `// Oathlord - explanation` or in YML, `# Oathlord - explanation`.
This should basically be a single-line diff explaining what you changed, e.g. `// Oathlord - removed Access` would clearly mean the `[Access]` attribute on a class was removed.
If you are changing a value say what it used to be, and optionally why it was changed. e.g. `attackRate: 1 # Oathlord - was 2, nerfed for being op`

For multi-line changes or replacements use the tag-like `// <Oathlord>` `// </Oathlord>` comment style.
When removing entire sections of code use `/* Oathlord` ... `*/`, assuming there are no multiline comments inside of that code.
